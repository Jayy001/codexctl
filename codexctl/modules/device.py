...
class DeviceManager:
    ...
    def get_device_status(self) -> tuple[str | None, str, str]:
        ...
        beta_possible = re.search("(?<=BetaProgram=).*", config_contents)
        prev_possible = re.search("(?<=[Pp]reviousVersion=).*", config_contents)

        prev = prev_possible.group() if prev_possible is not None else "unknown"
        beta = True if beta_possible is not None and beta_possible.group() else False

        current = re.search(
            "(?<=REMARKABLE_RELEASE_VERSION=).*", version_contents
        ).group()

        return beta, prev, current
...
/sbin/fw_setenv "active_partition" "${NEWPART}\""""
    ...
    def install_manual_update(self, version_available):
        ...
        if self.edit_update_conf(server_ip=server_host, server_port=8080) is False:
